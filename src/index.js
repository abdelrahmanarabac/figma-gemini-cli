...
async function getNextFreeX(gap = 100) {
  try {
    const result = await figmaEval(`(function() {
      let maxX = 0;
      figma.currentPage.children.forEach(n => {
        maxX = Math.max(maxX, n.x + n.width);
      });
...
async function getNextFreeY(gap = 100) {
  try {
    const result = await figmaEval(`(function() {
      let maxY = 0;
      figma.currentPage.children.forEach(n => {
...
program
  ...
  .action(async (jsxArrayStr, options) => {
    ...
    try {
      ...
      let currentX = vertical ? 0 : await getNextFreeX(gap);
      let currentY = vertical ? await getNextFreeY(gap) : 0;
      let results = [];
      ...
      for (const jsx of jsxArray) {
        try {
          ...
          try {
            if (vertical) {
              const height = await figmaEval(`(async () => { const n = await figma.getNodeByIdAsync('${result.id}'); return n ? n.height : 200; })()`);
              currentY += (height || 200) + gap;
            } else {
              const width = await figmaEval(`(async () => { const n = await figma.getNodeByIdAsync('${result.id}'); return n ? n.width : 300; })()`);
...
program
  ...
  .action(async (code, options) => {
    ...
    try {
      const result = await figmaEval(jsCode);
      if (result !== undefined && result !== null) {
        console.log(typeof result === 'object' ? JSON.stringify(result, null, 2) : result);
      }
    } catch (error) {
