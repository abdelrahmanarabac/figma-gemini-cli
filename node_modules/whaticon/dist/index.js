...
function normalizeSvgColors(svg) {
    ...
    function walk(node) {
        ...
        if (props) {
            if (props.fill && WHITE_COLORS.has(props.fill.toLowerCase()))
                props.fill = 'black';
            if (props.stroke && WHITE_COLORS.has(props.stroke.toLowerCase()))
                props.stroke = 'black';
        }
...
export async function svgToPixels(svg, size = DEFAULT_SIZE) {
 ...
 */
export function pixelSimilarity(px1, px2) {
 ...
 */
function computeHashFromPixels(data, size, rowWidth) {
 ...
 */
export async function computeDHash(svg, size = DEFAULT_SIZE) {
    const svgStr = normalizeSvgColors(typeof svg === 'string' ? svg : svg.toString('utf-8'));
    const input = Buffer.from(svgStr);
    const data = await sharp(input)
        .flatten({ background: '#ffffff' })
 ...
 */
function hammingDistanceAtOffset(h1, indexHashes, offset) {
    let distance = 0;
    for (let i = 0; i < HASH_BYTES; i += 4) {
        let xor = ((h1[i] ^ indexHashes[offset + i]) |
 ...
 */
export async function fetchIconSvg(name) {
 ...
 */
export function loadIndex(namesGz, hashesGz) {
 ...
 */
export async function findMatches(svg, index, options = {}) {
 ...
 */
function createSpriteSheet(icons, cols, iconSize) {
 ...
 */
function extractHashesFromSprite(data, spriteWidth, count, cols, iconWidth, iconHeight) {
    const hashes = [];
 ...
 */
export async function buildIndex(icons, size = DEFAULT_SIZE, onProgress) {
    const names = [];
    const hashBuffers = [];
    ...
    for (let i = 0; i < icons.length; i += batchSize) {
        ...
        try {
            ...
            const data = await sharp(Buffer.from(sprite))
                ...
                .toBuffer();
            // Extract hashes
            const batchHashes = extractHashesFromSprite(data, spriteWidth, batch.length, SPRITE_COLS, iconWidth, iconHeight);
